...
if __name__ == "__main__":
    ...
    for i in range(0, len(food_list)):
        print(f"2D Distance from food0 to food{i}: {food_list[0].distance_to_2d(food_list[i]):.4f}")
        print(f"3D Distance from food0 to food{i}: {food_list[0].distance_to_3d(food_list[i]):.4f}")
        print(f"Scaler adjustment from food0 to food{i}: {food_list[0].get_scalar(food_list[0].distance_to_2d(food_list[i]), food_list[0].z, food_list[i].z):.4f}")
        print(f"Energy cost from food0 to food{i}: {food_list[0].energy_cost_to(food_list[i]):.4f}\n")